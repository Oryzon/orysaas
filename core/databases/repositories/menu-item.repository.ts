...
export const MenuItemRepository = dataSource.getRepository(MenuItemEntity).extend({
    ...
    async getItemToMove(uuidMenu: string, uuidParent: string, order: string, position: number) {
        let query = this.createQueryBuilder("i").andWhere({ menuUuid: Equal(uuidMenu) });
        ...
        if (order === "up") {
            query.andWhere("i.position < :position", { position }).orderBy("i.position", "DESC");
        } else {
            query.andWhere("i.position > :position", { position }).orderBy("i.position", "ASC");
        }
