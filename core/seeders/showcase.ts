...
export async function showcaseSeeders() {
    ...
    await createDefaultPage(
        "Changelog",
        "Bienvenue sur une page par défaut, n'oubliez pas de mettre a jour le contenu.",
        "changelog",
    );
    await createDefaultPage(
        "Roadmap",
...
async function createMenuItem(
    ...
    url: string,
    position: number,
    parent: MenuItemEntity | null = null,
...
async function createFunctionnality() {
    ...
    await createHeroBloc(
        page,
        0,
        "Tout ce qu'il faut, rien de plus.",
        "Les briques qui prennent 3 mois à monter sont là dès le clone.",
        { text: "", url: "", variant: "flat" },
    );
...
async function createFaq() {
    ...
    await createHeroBloc(
        ...
        "Vous avez 30 secondes ? On répond aux dix plus fréquentes.",
        { text: "", url: "", variant: "flat" },
    );
...
async function createMultiCards(
    page: PageEntity,
    order: number,
    cards: Array<{
        title: string;
