            ...
            try {
                ...
                switch (true) {
                    case typeof error === "object" && error?.error === 100:
                        if (error.error === 100) {
                            response.status(HttpCode.MULTIPLE_CHOICE).send({
                                code: 300,
                    ...
                    case typeof error === "string":
                        response.status(HttpCode.UNPROCESSABLE_ENTITY).send({ message: error });

                        break;
                    case error instanceof EntityNotFoundError: {
                        const entityName =
                            typeof error.entityClass === "function"
                    ...
                    default:
                        ...
                        response.status(HttpCode.INTERNAL_ERROR).send({ message: "Something wrong happened." });
                }
            }
        };
