                                ...
                                <v-card-text>
                                    <v-row align="center" justify="center">
                                        ...
                                        <v-col md="12" class="mt-n4 mb-n2" v-if="priceFor(plan)">
                                            <span class="text-display-large font-weight-black">{{
                                                $price(priceFor(plan)!.sellPrice)
                                            }}</span>
                                            <span class="text-title-medium">
                                                / {{ selectedInterval === BillingInterval.MONTH ? "mois" : "an" }}</span
                                            >
                                            <v-chip
                                                v-if="priceFor(plan)?.discount"
                                                color="success"
                                                size="small"
                                                class="ml-2"
                                        ...
                                        <v-col md="8">
                                            ...
                                            >
                                                ...
                                                <span v-else-if="priceFor(plan)!.sellPrice === 0"
                                                    >Commencer gratuitement</span
                                                >
                                                <span v-else-if="priceFor(plan)!.trialPeriod > 0"
                                                    >Démarrer {{ priceFor(plan)!.trialPeriod }}j d'essai</span
                                                >
...
const { data: plans, pending: isLoading } = await useFetch<Plan[]>(`${runtime.public.apiBase}plans/public`, {
    key: `page:plans`,
});
...
useSeoMeta({
    ...
    ogDescription:
        "Auth, paiements, équipes, admin, emails. Tout est déjà branché. Vous n'écrivez plus que la partie qui rend votre produit unique.",
