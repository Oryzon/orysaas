<template>
    <v-dialog
        ...
        v-if="props.entity.isEnabled && props.entity.isRegistered"
    >
        <template v-slot:activator="{ props: activatorProps }">
            <v-btn v-bind="activatorProps" variant="text" icon color="success">
            ...
            <v-card :loading="isLoading">
                ...
                <v-card-text>
                    <v-row class="mt-6" v-if="lastRun.input">
                        <v-col md="12" v-for="[key, value] in Object.entries(lastRun.input)" class="mt-n8">
                            <v-text-field
                                :label="key"
                                variant="outlined"
                ...
                <v-card-actions class="bg-surface-light mt-n2">
                    ...
                    <v-btn color="success" variant="flat" @click="handleRun">Lancer</v-btn>
                </v-card-actions>
            </v-card>
        </template>
    </v-dialog>
...
watch(
    ...
    (run) => {
        if (run?.input) {
            nextRunInput.value = Object.fromEntries(Object.keys(run.input).map((key) => [key, null]));
        }
    },
    { immediate: true },
);
