<template>
    <v-dialog v-model="dialog" max-width="600" :persistent="isLoading || step === 'confirm'">
        <template #activator="{ props: activatorProps }">
            <v-btn color="error" variant="tonal" rounded="lg" min-width="180" v-bind="activatorProps">
        ...
        <v-card flat>
            <v-toolbar color="error" flat>
                <v-toolbar-title class="d-flex align-center ga-2">
                    ...
                    {{ step === "warn" ? "Supprimer mon compte" : "Confirmer la suppression" }}
                </v-toolbar-title>
            ...
            <template v-if="step === 'warn'">
                <v-card-text class="pa-6">
                    <v-alert type="warning" variant="tonal" rounded="lg" icon="mdi-alert-outline" class="mb-4">
                        ...
                        Votre profil sera anonymisé et vous serez retiré de toutes vos organisations. Les
                        organisations dont vous êtes l'unique membre seront supprimées avec leur abonnement.
                    </v-alert>
...
const isLoading = computed(() => api.isLoading("account:delete-request") || api.isLoading("account:delete-confirm"));
...
const sendCode = async () => {
    const res = await api.remove("/user/me/request", {
        loadingKey: "account:delete-request",
        toast: true,
    });
...
const confirm = async () => {
    ...
    const res = await api.remove("/user/me/confirm", {
        loadingKey: "account:delete-confirm",
        toast: true,
