            ...
            <v-card flat>
                ...
                <v-card-text>
                    <v-row>
                        <v-col md="12">
                            <p>
                                Vous êtes sur le point d'archiver le message de
                                <strong>{{ entity?.lastname }} {{ entity?.firstname }}</strong
                                >.
                            </p>
                            <p>
                                Ce message a été reçu le {{ $date.french(props.entity?.createdAt) }} et avait en sujet
...
const handleRemove = async () => {
    const res = await api.remove<{ message: string; entity: Contact }>(`/contact/${props?.entity?.uuid}`, {
