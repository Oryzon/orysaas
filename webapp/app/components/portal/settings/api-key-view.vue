<template>
    <v-dialog v-model="dialog" max-width="800" :persistent="isLoading">
        <template v-slot:activator="{ props: activatorProps }">
            <v-btn v-bind="activatorProps" variant="text" color="info" icon>
        ...
        <template v-slot:default>
            <v-card :loading="isLoading">
                <v-toolbar color="info">
                    <v-toolbar-title
                        >Voir la clé d'API <strong>{{ entity.label }}</strong></v-toolbar-title
                    >
...
watch(dialog, async (newVal) => {
    if (newVal) {
        decrypted.value = await api.get<string>(`/settings/api-key/${props.entity.uuid}`, {
