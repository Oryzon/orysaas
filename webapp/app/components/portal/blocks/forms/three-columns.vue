    ...
    <v-row>
        ...
        <v-col md="4">
            <v-row>
                <v-col md="8">
                    <v-text-field
                        v-model="localData.middleColumn.title"
                        label="Titre principal"
                        variant="outlined"
                        @update:model-value="emitUpdate"
                    ></v-text-field>
                ...
                <v-col md="4">
                    <v-select
                        v-model="localData.middleColumn.width"
                        label="Largeur de la colonne"
                        variant="outlined"
                        :items="widthSelect"
                        @update:model-value="emitUpdate"
                ...
                <v-col md="12" style="height: 500px;" class="mt-n8 mb-16">
                    <client-only>
                        <quill-editor
                            toolbar="full"
                            contentType="html"
                            v-model:content="localData.middleColumn.content"
                            @update:content="emitUpdate"
                        ></quill-editor>
                    </client-only>
                </v-col>
...
interface Props {
    modelValue: ThreeColumnsData;
}

const props = defineProps<Props>();
const emit = defineEmits<{
    'update:modelValue': [value: ThreeColumnsData];
}>();

const localData = ref<ThreeColumnsData>(JSON.parse(JSON.stringify(props.modelValue)));
