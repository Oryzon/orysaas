<template>
    ...
    <span v-if="password" class="password-strength__label" :class="`text-${current.vuetifyColor}`"
...
const score = computed(
    () => Object.values(criteriaSchemas).filter((schema) => schema.safeParse(props.password).success).length,
);
